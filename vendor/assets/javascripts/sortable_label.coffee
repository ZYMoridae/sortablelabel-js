      ...
      $(this).find(target).each( ->
        ...
        if typeof(_this.options['label']) == 'string'
          if _this.options['label'].trim() == 'Day'
            $(this).parent().find(_this.options['labelTarget']).html(_this.dayOfWeek(stepCount - 1))   
            stepCount += 1         
          else
            if _this.options['fieldName']
              if $(this).parent().find('.remove_nested_fields').data('association') == _this.options['fieldName']
                $(this).parent().find(_this.options['labelTarget']).html(_this.options['label'] + stepCount)
                stepCount += 1
            else
              throw "Field name can't be empty!"
        else if typeof(_this.options['label']) == 'function'
          $(this).parent().find(_this.options['labelTarget']).html(_this.options['label'].call($(this).closest('.fields').find(_this.options['labelTarget']), stepCount))
          stepCount += 1
